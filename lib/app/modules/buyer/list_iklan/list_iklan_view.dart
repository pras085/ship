...
import 'package:muatmuat/app/template/widgets/dealer_brand/dealer_brand_buyer_controller.dart';
...
class ListIklanView extends StatefulWidget {
  @override
  _ListIklanViewState createState() => _ListIklanViewState();
}

class _ListIklanViewState extends State<ListIklanView> {

  final controller = Get.put(ListIklanController());
  Map menu;
  ...
  void initState() {
    super.initState();
    menu = Get.arguments;
    menu['subKategori'] = menu['subKategori'].toString().replaceAll(" and ", " & ");
    controller.argument.value = menu;
    if (
      "${controller.argument.value['ID']}" == '23' // Dealer & Karoseri | Dealer
      || "${controller.argument.value['ID']}" == '24' // Dealer & Karoseri | Katalog Produk Dealer
    ) {
      // init brandController for dealer and katalog produk dealer
      Get.put(DealerBrandBuyerController());
    }
                        ...
                        if (
                          ...
                          DealerBrandBuyer(
                            ...
                            onSelectedData: (data) {
                              if (
                                controller.brand.value != null
                                && controller.brand.value.brandID == data.brandID
                              ) {
                                controller.brand.value = null;
                              } else {
