...
class SearchLocationMapMarkerDataPribadiController extends GetxController {
  ...
  void onInit() {
    ...
    if (Get.arguments != null) {
      ...
      informationMarker.value.formattedAddress = Get.arguments[addressKey] ?? "";
      ...
      _isFirstTimeOnChangePosition = informationMarker.value.formattedAddress != "";
    }
  }
  ...
  Future _getInformationFromLatLng() async {
    List<geoc.Placemark> placemark = await geoc.placemarkFromCoordinates(_latLngPositionMarker.latitude, _latLngPositionMarker.longitude);
    currentLang.value = _latLngPositionMarker.latitude;
    currentLong.value = _latLngPositionMarker.longitude;
    if (placemark.length > 0) {
      ...
      //         ";end",
      //     Get.context);
      String street = placemark[0].thoroughfare == "" && placemark[0].subThoroughfare == ""
          ? placemark[0].street
          : (placemark[0].thoroughfare + " " + placemark[0].subThoroughfare);
      if (placemark[0].subLocality.isNotEmpty) street += ", " + placemark[0].subLocality;
      if (placemark[0].locality.isNotEmpty) street += ", " + placemark[0].locality;
      if (placemark[0].subAdministrativeArea.isNotEmpty) street += ", " + placemark[0].subAdministrativeArea;
      if (street.split(",").length < 3 || placemark[0].isoCountryCode != "ID") {
        ...
        informationMarker.value.latLng = LatLng(_latLngPositionMarker.latitude, _latLngPositionMarker.longitude);
        informationMarker.refresh();
      }
    } else
  ...
  void onApplyButton() async {
    ...
    if (informationMarker.value.formattedAddress != "") Get.back(result: [_latLngPositionMarker, informationMarker.value.formattedAddress, districtid.value]);
  }
}
