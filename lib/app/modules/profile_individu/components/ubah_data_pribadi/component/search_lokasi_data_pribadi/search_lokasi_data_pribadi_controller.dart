...
import 'package:muatmuat/app/modules/profile_individu/components/ubah_data_pribadi/component/search_location_map_marker_data_pribadi/search_location_map_marker_data_usaha_controller.dart';
...
class SearchLocationDataPribadiController extends GetxController {
    ...
    _searchAddressGoogleFunction = SearchAddressGoogleFunction(
        ...
        getResultListAddress: (list) {
          ...
          for (AddressGooglePlaceAutoCompleteModel data in list) {
            listSearchAddress.add(AddressGoogleInfoPermintaanMuatModel(addressAutoComplete: data));
          }
        },
  ...
  Future _getListHistorySearch() async {
    try {
      var resultJson = jsonDecode(await SharedPreferencesHelper.getHistorySearchLocationInfoPermintaanMuat()) as List;
      listSearchAddress.addAll(resultJson.map((data) => AddressGoogleInfoPermintaanMuatModel.fromJson(data)).toList());
  ...
  void onClickAddress(int index) {
    ...
    if (listSearchAddress[index].addressDetails != null) {
      ...
      _searchAddressGoogleFunction.onClickListAddress(listSearchAddress[index].addressAutoComplete.placeId);
    }
  ...
  void onCompleteBuildWidget() async {
    if (_firstTimeBuildWidget) {
      ...
      searchTextEditingController.value.selection = TextSelection(baseOffset: 0, extentOffset: searchTextEditingController.value.text.length);
  ...
  dynamic _setMapLokasi(String nama, LatLng lokasi, String city, String district) {
    return {namaKey: nama, lokasiKey: lokasi, cityKey: city, districtKey: district};
  }

  void _whenGetDetail(AddressGooglePlaceDetailsModel detail) async {
    _addToSharedPref(AddressGoogleInfoPermintaanMuatModel(addressAutoComplete: listSearchAddress[_indexOnClick].addressAutoComplete, addressDetails: detail));
    ...
    _getBackWithResult(listSearchAddress[_indexOnClick].addressAutoComplete.description, detail.latLng, detail.districtName, detail.cityName,
        listSearchAddress[_indexOnClick].addressAutoComplete.placeId);
    //  Get.back(result: LokasiUbahDataController().selectLokasi.value);
    //  return Future.value(false);
  ...
  Future _getListSaveLocation() async {
    ...
    if (listSaveLocation != null) {
      listSaveLocation.clear();
      try {
        ManajemenLokasiResponseModel response = await ManajemenLokasiAPI.getLocation(GlobalVariable.userModelGlobal.docID);
  ...
  Future _getListHistoryTransactionLocation() async {
    try {
      HistoryTransactionLocationInfoPermintaanMuatResponseModel response =
          await GetHistoryTransactionLocation.getLocation(GlobalVariable.userModelGlobal.docID);
      listHistoryTransactionLocation.addAll(response.listData);
    } catch (err) {}
  }

  void onClickSetPositionMarker() async {
    var result = await GetToPage.toNamed<SearchLocationMapMarkerDataPribadiController>(Routes.PETA_UBAH_DATA_PRIBADI, arguments: {
      SearchLocationMapMarkerDataPribadiController.latLngKey: _latLngFromArgms,
      SearchLocationMapMarkerDataPribadiController.imageMarkerKey: Stack(alignment: Alignment.topCenter, children: [
        Image.asset(
          'assets/pin_new.png',
  ...
  void onClickListSaveLocation(int index) {
    _getBackWithResult(listSaveLocation[index].address, LatLng(listSaveLocation[index].latitude, listSaveLocation[index].longitude),
        listSaveLocation[index].district, listSaveLocation[index].city, listSearchAddress[index].addressAutoComplete.placeId);
  }
  ...
  void onClickSaveLocation({String address, String placeID}) async {
    var result = await GetToPage.toNamed<EditManajemenLokasiInfoPermintaanMuatController>(Routes.EDIT_MANAJEMEN_LOKASI_INFO_PERMINTAAN_MUAT,
        arguments: {"Address": address, "placeIDKey": placeID, "TypeEditManajemenLokasiInfoPermintaanMuat": TypeEditManajemenLokasiInfoPermintaanMuat.ADD});
  ...
  void onClickEditSaveLocation(int index) async {
    var result = await GetToPage.toNamed<EditManajemenLokasiInfoPermintaanMuatController>(Routes.EDIT_MANAJEMEN_LOKASI_INFO_PERMINTAAN_MUAT, arguments: {
      EditManajemenLokasiInfoPermintaanMuatController.manajemenLokasiModelKey: listSaveLocation[index],
      EditManajemenLokasiInfoPermintaanMuatController.typeEditManajemenLokasiInfoPermintaanMuatKey: TypeEditManajemenLokasiInfoPermintaanMuat.UPDATE
    });
    if (result != null) if (result) _getListSaveLocation();
    Timer(Duration(milliseconds: 300), () {
      Get.delete<EditManajemenLokasiInfoPermintaanMuatController>();
  ...
  void onClickListHistoryTransactionLocation(int index) {
    _getDataInfoFromAddressPlaceID(address: listHistoryTransactionLocation[index].address);
  }
  ...
  Future _getDataInfoFromAddressPlaceID({@required String address, String placeID}) async {
    ...
    _addToSharedPref(AddressGoogleInfoPermintaanMuatModel(
        ...
        addressDetails: AddressGooglePlaceDetailsModel(
            formattedAddress: address, latLng: LatLng(response.latitude, response.longitude), cityName: response.city, districtName: response.district),));
  ...
  void _getBackWithResult(String address, LatLng latLng, String district, String city, String placeid) {
    onClearSearch();
  ...
  void goToListAllManajemenLokasi() async {
    var result = await GetToPage.toNamed<SearchManajemenLokasiInfoPermintaanMuatController>(Routes.SEARCH_MANAJEMEN_LOKASI_INFO_PERMINTAAN_MUAT);
    _getListSaveLocation();
    if (result != null) _getBackWithResult(result[0], result[1], result[2], result[3], result[4]);
  }
}
