...
import 'package:muatmuat/app/modules/home/home/logistik/transport_market/subscription/alert_dialog/tm_subscription_popup_konfirmasi_pembatalan_pembayaran.dart';
import 'package:muatmuat/app/modules/home/home/logistik/transport_market/subscription/api_tm_subscription.dart';
import 'package:muatmuat/app/modules/home/home/logistik/transport_market/subscription/models/tm_subscription_detail_paket_model.dart';
...
class TMSubscriptionDetailController extends GetxController {
  ...
  int id;
  TMTipeLayananSubscription tipe;
  TMTipeDetailSubscription tipeDetail;
  List<TMSubscriptionDetailPaketModel> listData = [];
  String kodePesanan = '';
  ...
  void onInit() async {
    ...
    hasAccessDownloadRiwayatPesanan.value = await CekSubUserDanHakAkses().cekSubUserDanHakAksesWithShowDialog(context: Get.context, menuId: "487", showDialog: false);
    hasAccessShareRiwayatPesanan.value = await CekSubUserDanHakAkses().cekSubUserDanHakAksesWithShowDialog(context: Get.context, menuId: "488", showDialog: false);
  ...
  getData() async {
    ...
    var response;
    if (tipe == TMTipeLayananSubscription.BF) {
      response = await ApiTMSubscription(
    ...
    if (message != null && message.code == 200) {
      ...
      data.forEach((element) {
        listData.add(TMSubscriptionDetailPaketModel.fromJson(element));
  ...
  void bayarSekarang() async {
    ...
    var resultPayment =
        await GetToPage.toNamed<TMPembayaranSubscriptionController>(
          ...
          totalHarga,
          batasPembayaran,
          tipe == TMTipeLayananSubscription.BF
          ...
          pajak,
          tipe == TMTipeLayananSubscription.SU,
          id,
          isNext
        ]);
    ...
    if (resultPayment != null && resultPayment[1] == "justRefresh") {
      ...
      if ((resultPayment[0] is String) && (resultPayment[0] as String).length > 0) {
        ...
        CustomToast.show(
          context: Get.context,
          message: tipe == TMTipeLayananSubscription.SU
  ...
  void showBatalDialog() {
    TMSubscriptionPopupKonfirmasiPembatalanPembayaran.showAlertDialog(
        context: Get.context);
  }
  // void showBatalDialog() {
  //   var selected = "0".obs;
  ...
  void batalOrder(String pesan) async {
    ...
    var result = tipe == TMTipeLayananSubscription.BF
  ...
  void downloadFile() async {
    try {
      ...
      if (status.isGranted) {
        ...
        var idUser = await SharedPreferencesHelper.getUserShipperID();
        var pdfURL = await getPDFLink(
            // "${ApiHelper.urlzo}generate/faktur_subscription_tm/?idx=${GlobalVariable.docID}&role=2&id=${id}&type=${tipe == TMTipeLayananSubscription.BF ? "0" : "1"}&app=1");
            "${ApiHelper.urlzo}generate/tm_subscription_view_faktur_pdf?role=2&id=${id}&type=${tipe == TMTipeLayananSubscription.BF ? "0" : "1"}&idLogin=$idUser&app=1&email=${GlobalVariable.emailLogin}");
