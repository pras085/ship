...
class TMCreateSubscriptionController extends GetxController {
  ...
  void checkFirst() async {
    if (metodePembayaran.isEmpty) {
      ...
      try {
        var dataCek = TMCheckSegmentedUserModel.fromJson(response['Data']);
        if (dataCek.isFirst == 1) {
          loadingCreate.value = false;
          var result = await GetToPage.toNamed<
                  TMTermsAndConditionsSubscriptionController>(
              Routes.TM_TERMS_AND_CONDITIONS_SUBSCRIPTION);
  ...
  void showInfoKadaluarsa() {
    GlobalAlertDialog.showAlertDialogCustom(
        ...
        message: "SubscriptionAlertSubscriptionNext"
            .tr
