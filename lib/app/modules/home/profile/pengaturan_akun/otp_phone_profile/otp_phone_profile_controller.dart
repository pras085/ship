...
import 'package:muatmuat/app/modules/home/profile/pengaturan_akun/otp_phone_profile/verify_phone_check_status_response_model.dart';
import 'package:muatmuat/app/modules/home/profile/pengaturan_akun/otp_phone_profile/verify_phone_response_model.dart';
import 'package:muatmuat/app/network/api_helper.dart';
import 'package:muatmuat/global_variable.dart';

class OtpPhoneProfileController extends GetxController {
  ...
  cekWaktuOtp(String type) async {
    ...
    var response = await ApiProfile(context: Get.context, isShowDialogLoading: false).getTimerSetting(body);
  ...
  verifyOtp() async {
    ...
    if (otp.length == otpLength) {
      ...
      if (message != null && message.code == 200) {
        ...
        // update user model
        log("No WA Current: " + phoneNumber.value);
        log("No WA Change: " + changePhoneNoValue.value);

        GlobalVariable.userModelGlobal.phone = changePhoneNoValue.value;
        GlobalVariable.userModelGlobal.phone = phoneNumber.value;
  ...
  changePhoneNumber() async {
    ...
    log("No WA Change: " + changePhoneNoValue.value);
    var body = {
      "phone_baru": changePhoneNoValue.value,
  ...
  Future _checkVerifyPhone({bool isStartTimerWhenNotVerify = true}) async {
    ...
    var responseBody = await apiHelperCheckVerifyPhone.fetchVerifyPhone(userModel.value.docID, userModel.value.phone);
    if (responseBody == null) {
      isError.value = true;
      errorMessage.value = apiHelperCheckVerifyPhone.errorMessage;
    } 
    else {
      VerifyPhoneCheckStatusResponseModel verifyResponse = VerifyPhoneCheckStatusResponseModel.fromJson(responseBody);
  ...
  Future checkVerifyOTP() async {
    ...
    if (otp.length == otpLength) {
      ...
      var reponseBody = await apiHelperCheckOTPPhone.fetchVerifyOTPPhone(verifID, otp);
      if (reponseBody != null) {
  ...
  Future sendVerifyPhone() async {
    ...
    var reponseBody = await apiHelper.fetchResendVerifyPhone(userModel.value.docID);
    if (reponseBody != null) {
      VerifyPhoneResponseModel verifyResponse = VerifyPhoneResponseModel.fromJson(reponseBody);
      if (verifyResponse.message.code == 200) {
