...
class ManajemenNotifikasiAplikasiController extends GetxController {
        ...
        if (response["Message"]["Code"] == 200) {
          ...
          if(response["Data"]["Categories"].length > 0) {
            ...
            for(int i = 0; i < response["Data"]["Categories"].length; i++) {
              ...
              log((i+1).toString() + ". CATEGORY NAME: " + temp.categoryName + " - ID: " + temp.categoryIdentifier.toString() + ", SUB CATEGORY NAME: " + temp.subCategoryName + " - ID: " + temp.subCategoryIdentifier.toString() + ", Email Notif: " + temp.pushNotif.toString());
  ...
  Future<void> setNotifSetting({
    ...
    if(model.categoryIdentifier == 1) {
      model.pushNotifToggle.value = toggle;
    }
    model.pushNotif = toggle ? 1 : 0;

    log("TOGGLE: " + model.pushNotifToggle.value.toString());
    log("VALUE: " + model.pushNotif.toString());
  ...
  checkToggleAllNotif() {
    ...
    for(int i = 0; i < listModelManajemenVerifikasiGeneral.length; i++) {
      if(!listModelManajemenVerifikasiGeneral[i].pushNotifToggle.value) {
    ...
    for(int i = 0; i < listModelManajemenVerifikasiBf.length; i++) {
      if(!listModelManajemenVerifikasiBf[i].pushNotifToggle.value) {
        allNotifEnabled = false;
    ...
    for(int i = 0; i < listModelManajemenVerifikasiTm.length; i++) {
      if(!listModelManajemenVerifikasiTm[i].pushNotifToggle.value) {
        allNotifEnabled = false;
