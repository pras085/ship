...
class ListHeadCarrierController extends GetxController {
  ...
  void onInit() {
    ...
    selectedData.value = Get.arguments[1].toString();
    if (type.value == "1") selectHead = Get.arguments[2].toString();
  ...
  doFilter(int offset) async {
    var resultFilter = (type.value == "0")
        ? await ApiHelper(context: Get.context, isShowDialogLoading: false)
            .listHeadTruck()
        : await ApiHelper(context: Get.context, isShowDialogLoading: false)
            .fetchCarrierTruck(headID: "0"); //headID: selectHead.toString()
    // .listCarrierTruck();
    ...
    if (type.value == "0") {
      data.add({
        "ID": "Semua",
        "Description": "Semua Truk",
        "ImageHead": "",
      });
    } else {
      data.add({
        "ID": "Semua",
        "Description": "Semua Carrier",
        "ImageHead": "",
      });
    }
    for (var x = 0; x < resultFilter["Data"].length; x++) {
      data.add(resultFilter["Data"][x]);
    }
