...
class ListHeadCarrierView extends GetView<ListHeadCarrierController> {
  @override
    ...
    return Container(
      ...
      children: [
        GestureDetector(
          onTap: () {
            controller.selectedData.value =
                controller.filteredData[index]["ID"].toString();

            if (!controller.showButtonSave) {
              var data = List.from(controller.listData).firstWhere((element) =>
                  element["ID"].toString() ==
                  controller.selectedData.value.toString());
              controller.filterSearch.value = "";
              controller.onClearSearch();
              Get.back(result: data);
            }
