...
class DetailProsesTenderController extends GetxController {
  ...
  var sudahAdaPemenang = false.obs;
  Timer time;
  ...
  var kodeTender = 'ProsesTenderCreateLabelAutoGenerate'.obs;
  var judulTender = "".obs;
  ...
  var namaTahapTender = [
    "",
    "ProsesTenderDetailLabelProsesTender".tr, // Info Pra Tender
    "ProsesTenderDetailLabelProsesTender".tr, // Proses Tender
    "ProsesTenderDetailLabelSeleksiPemenang".tr, // Seleksi Pemenang
    "ProsesTenderDetailLabelPengumumanPemenang".tr, // Pengumuman Pemenang
    "ProsesTenderDetailLabelEksekusiTender".tr //Eksekusi Tender
  ];
  ...
  void onInit() async {
    ...
    cekShare = await SharedPreferencesHelper.getHakAkses(
        "Export Detail Proses Tender");
    cekKirim = await SharedPreferencesHelper.getHakAkses(
    ...
    updateTitle();
    idTender = Get.arguments[0].toString();
  ...
  Future getDetail(String keterangan) async {
    ...
    if (result['Message']['Code'].toString() == '200') {
      ...
      if (keterangan == "") {
        ...
        sudahAdaPemenang.value = data['hasWinner'] ?? false;

        if (jenisTab == 'Aktif') {
          //BATAL JADI HISTORY
          if (status.value.toString() == "3" ||
              DateTime.parse(data['tahap_tender'][1]['tanggal_akhir'] +
                          " 23:59:59")
                      .compareTo(dateNow) <
                  0) {
            jenisTab = "History";
        ...
        if (dataTahapTender[2]['min_date'].compareTo(dateNow) <= 0) {
          masaSeleksi = true;
        } else {
          masaSeleksi = false;
        }
    ...
    if (satuanTender == 2) {
      switch (slideIndex.value) {
        case 0:
          {
            title.value =
                'ProsesTenderDetailLabelDataProsesTender'; // Data Proses Tender
            break;
          }
        case 1:
          {
            title.value = 'ProsesTenderDetailLabelDataMuatan'; // Data Muatan
            break;
          }
        case 2:
          {
            title.value = 'ProsesTenderDetailLabelDataTender'; // Data Tender
            break;
          }
        case 3:
          {
            title.value =
                'ProsesTenderDetailLabelRuteYangDitenderkan'; // Rute yang Ditenderkan
            break;
          }
        case 4:
          {
            title.value =
                'ProsesTenderDetailLabelPersyaratanKualifikasi'; // Syarat dan Ketentuan
            break;
          }
      }
    } else {
  ...
  void shareDetailProsesTender() async {
    String id = await SharedPreferencesHelper.getUserShipperID();
    onProgress.value = 0;
    showDialog(
        context: Get.context,
    ...
    var result = await ApiHelper(
            ...
            isShowDialogLoading: false,
            isShowDialogError: false)
        .sharePDFDetailProsesTender(id, idTender);
    ...
    return Column(
      ...
      children: [
        Row(
          ...
          children: [
            Container(
              child: CustomText('ProsesTenderDetailLabelPeriode'.tr, // Periode
                  fontWeight: FontWeight.w600,
                  fontSize: 12,
                  height: 1.3,
            ...
            Container(
              child: CustomText(
                  ' ' + 'ProsesTenderDetailLabelSampaiDengan'.tr + ' ', //s/d
                  fontSize: 12,
                  textAlign: TextAlign.center,
                  color: Colors.black,
  ...
  Future kirimUlang(int idEmail, int index) async {
    ...
    var result = await ApiHelper(
            ...
            isShowDialogLoading: false,
            isShowDialogError: false)
        .resendEmailProsesTender(shipperID, idEmail.toString());
  ...
  void umumkan(String idTender) async {
    GlobalAlertDialog.showAlertDialogCustom(
        context: Get.context,
        title: "PemenangTenderIndexLabelUmumkanPemenang".tr, //Umumkan Pemenang
