...
import 'create_proses_tender_view.dart';

class CreateProsesTenderController extends GetxController {
  ...
  var namaTahapTender = [
    "",
    "ProsesTenderCreateLabelInfoPraTender", //Info Pra Tender
  ...
  Future<void> onInit() async {
    ...
    if (Get.arguments != null) {
      ...
      await getDetail(idTender, Get.arguments[1]);
    } else {
  ...
  Future getDetail(String idtrans, int halaman,
    ...
    if (jenisTransaksi == "TD") {
      ...
      result = await ApiHelper(
              ...
              isShowDialogLoading: false,
              isShowDialogError: false)
          .getDetailInfoPraTender(id, idtrans);
    }
    ...
    if (result['Message']['Code'].toString() == '200') {
      ...
      for (var x = 0; x < data['tahap_tender'].length; x++) {
        if (jenisTransaksi == "TD") {
          ...
          var tglawal = 'ProsesTenderCreateLabelFirstDate'.tr;
          var tglakhir = 'ProsesTenderCreateLabelLastDate'.tr;
          var minDate = null;
          var maxDate = null;
          if (DateTime.parse(
                      data['tahap_tender'][x]['tanggal_dimulai'] + " 23:59:59")
                  .compareTo(
                      await GlobalVariable.getDateTimeFromServer(Get.context)) >
              0) {
            print('TES TGL AWAL');
            tglawal = data['tahap_tender'][x]['tanggal_dimulai_format_new'];
            minDate =
                DateTime.parse(data['tahap_tender'][x]['tanggal_dimulai']);
          }

          if (DateTime.parse(
                      data['tahap_tender'][x]['tanggal_akhir'] + " 23:59:59")
                  .compareTo(
                      await GlobalVariable.getDateTimeFromServer(Get.context)) >
              0) {
            print('TES TGL AKHIR');
            tglakhir = data['tahap_tender'][x]['tanggal_akhir_format_new'];
            maxDate = DateTime.parse(data['tahap_tender'][x]['tanggal_akhir']);
          }

          if (data['tahap_tender'][x]['tahap_tender'] == 5) {
            dataTahapTender.add({
              'tahap_tender': data['tahap_tender'][x]['tahap_tender'],
              'show_first_date': tglawal,
              'show_last_date': tglakhir,
              'min_date': minDate ?? '',
              'max_date': maxDate ?? '',
              'error_tahap_tender': '',
              'error_min_date':
  ...
  void onSetData(String tipe) async {
    ...
    if (tipe == "SAVE" && save) {
      ...
      if (result['Message']['Code'].toString() == '200') {
        ...
        CustomToast.show(
            context: Get.context,
            message: "ProsesTenderCreateLabelInfoBerhasilMenyimpanData"
                  ...
                  for (var indexDetail = 0;
                    ...
                    dataRuteTender[index]['data'][indexDetail]['nilai'] != 0
                        ? Container(
                            ...
                            /// JIKA INDEX TERAKHIR, TIDAK PERLU
                            child: Row(
                              mainAxisAlignment: MainAxisAlignment.spaceBetween,
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
  ...
  void lihatInformasiTenderTertutup(String jenisTertutup) async {
    FocusManager.instance.primaryFocus.unfocus();
