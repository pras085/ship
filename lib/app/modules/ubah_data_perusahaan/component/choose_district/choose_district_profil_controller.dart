...
class ChooseDistrictProfilPerusahaanController extends GetxController {
  var districtNameKey = 'District';
  var districtIdKey = 'DistrictID';
  final UbahDataPerusahaanController ubahDataC = Get.find();
  ...
  void onInit() async {
    ...
    if (ubahDataC.distid.value == "") {
      ...
      ).fetchInformationLocationByToken(Get.arguments);
  ...
  Future search({String search = ""}) async {
    text.value = search;
    districtsTemp.value =
        text.value == "" ? districts : districts.where((e) => e[districtNameKey].toString().toLowerCase().contains(text.value.toLowerCase())).toList();
  }
